...
from . import Agent, UNIFORM, GREEDY, SOFTMAX
...
class TestAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # dummy approximator (see appproximator)
        class Approx:
            ...
            def predict(self, x):
                sind = int(10 + x[:, 0])
                aind = int(x[0, 1])
                return np.array((self.values[sind, aind],)).reshape(1,1)
            
            def update(self, x, y):
                # not really necessary since here `learn` is not being called.
                # For `learn` testing, see `algorithm.test`
                sind = int(10 + x[0, 0])
                aind = int(x[0, 1])
                self.values[sind, aind] = y[0, 0]       # learning rate == 1
